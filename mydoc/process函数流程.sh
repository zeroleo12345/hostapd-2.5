...
SM_STATE(EAP, METHOD_RESPONSE)      # 在主流程中!!! 处理客户端发过来的响应, 三个连续处理状态: RECEIVED -> INTEGRITY_CHECK -> METHOD_RESPONSE
{
...
static void eap_peap_process_msg(struct eap_sm *sm, void *priv, const struct wpabuf *respData)
{
	...
	case PHASE2_SOH:
	case PHASE2_TLV:
		eap_peap_process_phase2(sm, data, respData, data->ssl.tls_in);      # 调用下面
		break;
...
# src/eap_server/eap_server_peap.c
static void eap_peap_process_phase2_response(struct eap_sm *sm, struct eap_peap_data *data, struct wpabuf *in_data)		# 处理客户端发过来的报文
{
...
static void eap_peap_req_success(struct eap_sm *sm, struct eap_peap_data *data)
{
	if (data->peap_version == 0) {
		...
		eap_peap_state(data, PHASE2_TLV);		# PEAPv1 没有TLV!
	} else {
		eap_peap_state(data, SUCCESS_REQ);		# PEAPv0 直接到SUCCESS_REQ
	}
}